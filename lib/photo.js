...
/**
 * Obtain the original image palette
 * @param  {Photo} photo
 * @param  {Function} _callback
 */

function palette(photo, _callback) {

    couleur.getPalette(photo.path, 5, 10, function(err, palette) {
        if (err) {
            _callback(err);
        } else {
            photo.o.palette = palette;
            _callback();
        }
    });
